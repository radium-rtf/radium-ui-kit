...
export const List = forwardRef<HTMLUListElement, ListProps>(({className, asChild, ...props}, ref) => {
...
export const ListItem = forwardRef<HTMLLIElement, ListItemProps>(({className, asChild, ...props}, ref) => {
...
export const ListIcon = forwardRef<SVGSVGElement, ListIconProps>(({asChild, icon, className, ...props}, ref) => {
  const Comp = asChild
    ? (Slot as ForwardRefExoticComponent<
        SVGAttributes<SVGSVGElement> & RefAttributes<SVGSVGElement> & {children?: ReactNode}
      >)
    : Icon
  return <Comp {...props} type={icon || 'blank'} ref={ref} className={cn('shrink-0 text-accent-primary', className)} />
})
  ...
  ({asChild, icon, className, ...props}, ref) => {
    const Comp = asChild
      ? (Slot as ForwardRefExoticComponent<
        ...
        >)
      : SmallIcon
    return <Comp {...props} type={icon || 'blank'} ref={ref} className={cn('text-text shrink-0', className)} />
  }
)
...
export const ListContent = forwardRef<HTMLDivElement, IListContentProps>(({className, asChild, ...props}, ref) => {
  const Comp = asChild ? Slot : 'div'
  return <Comp ref={ref} {...props} className={cn('shrink grow space-y-0.5 overflow-hidden', className)} />
})
...
export const ListTitle = forwardRef<HTMLHeadingElement, IListTitleProps>(({className, asChild, ...props}, ref) => {
  const Comp = asChild ? Slot : 'h1'
  return <Comp ref={ref} {...props} className={cn('line-clamp-1 font-NTSomic  text-base leading-tight', className)} />
})
